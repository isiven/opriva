...
- 2026-06-02: Process decision — "Mandatory Agent-Review Section" documented as `AGENTS.md §21`. Every important Opriva audit, product/architecture/backend/security/data/UX decision, import, documents, relationships, permissions, dashboard, reports, AI or business-logic change must include an explicit "Revisión por agentes" section in the report, using the full 11-reviewer table (Product Manager senior, Enterprise SaaS Architect, Backend Readiness Auditor, CIO / IT Director, CISO / Security Auditor, IT Procurement / Vendor Management, Enterprise UX Auditor, Data Model / Import Architect, QA / Regression Auditor, Accessibility Auditor, Technical Debt / Refactor Auditor) with columns Aprueba / Preocupa / Recomienda / ¿Bloquea?, plus a panel verdict (Procede / Procede con condiciones / Bloqueado). Mechanical / low-risk tasks (push, commit, PR-readiness, simple housekeeping, minor docs-only) may use a proportional review or state "No aplica revisión completa por ser tarea mecánica." If any reviewer blocks, do not implement until resolved or Isaac approves. Operationalizes §13 and §26.6 (makes the review visible in reports, not just implied). No application code modified (docs-only); `source/App.jsx` and `source/` untouched.
