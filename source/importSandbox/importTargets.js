import { getMappedImportValue, getMappedImportValueAny } from './importMapping.js';
...
export function suggestImportTargetFromSource(sourceType) {
  ...
  if (sourceType === 'Hardware Sales Export') return 'Hardware / Warranty';
  if (sourceType === 'Commercial Renewal Package') return 'Mixed / Let Opriva classify rows';
  return 'Mixed / Let Opriva classify rows';
}

export function importTargetToModule(importTarget) {
  if (importTarget === 'Licenses') return { moduleKey: 'licenses', label: 'License' };
  if (importTarget === 'Hardware' || importTarget === 'Hardware / Warranty') return { moduleKey: 'hardware', label: 'Hardware / Warranty' };
  if (importTarget === 'Contracts / Support Coverage') return { moduleKey: 'contracts', label: 'Contract / Support Coverage' };
  if (importTarget === 'Certificates / SSL') return { moduleKey: 'licenses', label: 'Certificate / SSL' };
  if (importTarget === 'Renewal Package' || importTarget === 'Renewal Package / Bundle') return { moduleKey: 'package', label: 'Renewal Package / Bundle', review: true, warning: 'Renewal Package / Bundle import creates underlying local license, hardware or contract records in this sandbox.' };
  ...
  if (importTarget === 'Mixed / Multiple record types' || importTarget === 'Mixed / Let Opriva classify rows') return null;
  return null;
}
...
export function detectImportTarget(rowObj, mappings, sourceType, importTarget) {
  ...
  if (getMappedImportValue(rowObj, mappings, 'Serial Number') || getMappedImportValue(rowObj, mappings, 'Warranty End Date')) return { moduleKey: 'hardware', label: 'Hardware / Warranty' };
  if (getMappedImportValueAny(rowObj, mappings, ['Product / License Name','License / Product'])) return { moduleKey: 'licenses', label: 'License' };
  ...
  if (getMappedImportValue(rowObj, mappings, 'Expiration / Renewal Date')) return { moduleKey: 'licenses', label: 'License' };
