...
function Dashboard({ workspaceMode = 'MSP / Integrator', setWorkspaceMode = function(){}, onOpenAlerts = function(){}, onNavigate = function(){} }){
...
function CompaniesScreen({ workspaceMode = 'MSP / Integrator', onNavigate = function(){} }){
  const isInternalIT = workspaceMode === 'Internal IT';
  ...
  return <main className="content">
    ...
    <section className="panel worklistPanel">
      ...
      <div className="toolbar">
        ...
        <button type="button" disabled aria-disabled="true" title="Preview only — Opriva AI runs on the backend, not in the sandbox.">AI summary</button>
...
function TopbarShell({ active, onAlerts, onOpenCommand, onMenuToggle, onNavigate, workspaceMode = 'MSP / Integrator', setWorkspaceMode = function(){}, alertCount = 0 }){
  ...
  const accountWrapRef = React.useRef(null);
  ...
  React.useEffect(() => {
    const onDocClick = (e) => {
      ...
      if (accountWrapRef.current && !accountWrapRef.current.contains(e.target)) setAccountMenuOpen(false);
    };
    const onKey = (e) => { if (e.key === 'Escape') { setWorkspaceMenuOpen(false); setNewMenuOpen(false); setAccountMenuOpen(false); } };
    document.addEventListener('mousedown', onDocClick);
    document.addEventListener('keydown', onKey);
    return () => { document.removeEventListener('mousedown', onDocClick); document.removeEventListener('keydown', onKey); };
  }, []);

  const toggleWorkspace = () => { setNewMenuOpen(false); setAccountMenuOpen(false); setWorkspaceMenuOpen(o => !o); };
  const toggleNew = () => { setWorkspaceMenuOpen(false); setAccountMenuOpen(false); setNewMenuOpen(o => !o); };
  const toggleAccount = () => { setWorkspaceMenuOpen(false); setNewMenuOpen(false); setAccountMenuOpen(o => !o); };
  const goTo = (id) => { if (onNavigate) onNavigate(id); setNewMenuOpen(false); setAccountMenuOpen(false); };
  ...
  return <header className="topbar">
    ...
    <div className="tenantLockupWrap" ref={workspaceWrapRef}>
      ...
      {workspaceMenuOpen && <div className="topMenu workspaceMenu" role="menu">
        ...
        <button type="button" className="topMenuItem" disabled aria-disabled="true" title="Backend required — multiple tenant workspaces need a multi-tenant backend. In the sandbox, use the Mode selector to change operating mode.">
        ...
        <button type="button" className="topMenuItem" disabled aria-disabled="true" title="Backend required — managing tenant workspaces needs a multi-tenant backend. Not available in the sandbox.">
    ...
    <div className="topMenuWrap" ref={accountWrapRef}>
      <button type="button" className={cx('avatar', accountMenuOpen && 'avatarOpen')} onClick={toggleAccount} aria-expanded={accountMenuOpen} aria-haspopup="menu" aria-label="Account menu">MC</button>
      {accountMenuOpen && <div className="topMenu accountMenu" role="menu">
        <div className="topMenuHeader">
          <strong>Sandbox session</strong>
          <small>{workspaceLabel} · {workspaceMode}</small>
        </div>
        <button type="button" className="topMenuItem" role="menuitem" onClick={() => goTo('Settings')}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09a1.65 1.65 0 0 0-1-1.51 1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09a1.65 1.65 0 0 0 1.51-1 1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33h.01a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>
          Settings
        </button>
        <button type="button" className="topMenuItem" disabled aria-disabled="true" title="Backend required — accounts, profile and security need authentication and a backend.">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
          Account &amp; security
        </button>
        <button type="button" className="topMenuItem" disabled aria-disabled="true" title="Backend required — sessions are not persisted in the sandbox, so there is nothing to sign out of.">
...
// F4c: honest "New record" control. There is no real create-record modal in the
// sandbox yet, so instead of a single dead button we offer a type selector that
// routes to the matching canonical module (License / Hardware / Contract).
function NewRecordTypeMenu({ onNavigate = function(){}, label = 'New record' }){
  ...
  const types = [['License', 'Licenses'], ['Hardware', 'Hardware'], ['Contract', 'Contracts']];
  return <div className="topMenuWrap" ref={wrapRef} style={{display:'inline-block'}}>
    <button type="button" className="primary" onClick={() => setOpen(o => !o)} aria-expanded={open} aria-haspopup="menu">{label}
      <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" style={{marginLeft:6,verticalAlign:'middle'}}><path d="m6 9 6 6 6-6"/></svg>
    </button>
    {open && <div className="topMenu accountMenu" role="menu">
      <div className="topMenuHeader"><strong>{label}</strong><small>Choose a record type to create</small></div>
      {types.map(([t, route]) => <button key={t} type="button" className="topMenuItem" role="menuitem" onClick={() => go(route)}>{t}</button>)}
    </div>}
  </div>;
}

function AssetsRenewalsScreen({ workspaceMode = 'MSP / Integrator', onNavigate = function(){} }){
  const isInternalIT = workspaceMode === 'Internal IT';
  ...
  return <main className="content assetsRenewalsPage">
    <ScreenHeader active={isInternalIT ? 'Renewals Forecast' : 'Assets & Renewals'} eyebrow={isInternalIT ? 'RENEWALS FORECAST' : 'RENEWAL WORKLIST'} subtitle={isInternalIT ? 'Forecast upcoming IT renewals, department impact, brand/provider concentration and approval risk before spend becomes urgent.' : 'Manage tracked assets, licenses, contracts, warranties, SaaS subscriptions and certificates by urgency, value and ownership.'}>{isInternalIT ? <><button type="button" onClick={() => onNavigate('Data Import')}>Import records</button><button type="button" disabled aria-disabled="true" title="Backend required — saved forecast configuration needs persistence and a backend.">Configure forecast</button><NewRecordTypeMenu onNavigate={onNavigate} label="New renewal" /></> : <><button type="button" onClick={() => onNavigate('Data Import')}>Import records</button><button type="button" disabled aria-disabled="true" title="Backend required — saved column configuration needs persistence and a backend.">Configure columns</button><NewRecordTypeMenu onNavigate={onNavigate} label="New record" /></>}</ScreenHeader>
    ...
    {isInternalIT ? <section className="panel aiInsightBar assetsInsightBar"><p><strong>AI Insight (sample)</strong> Opriva detected $487K in upcoming IT renewals across 8 departments. Endpoint security shows provider overlap across Kaspersky, Symantec and McAfee. Review consolidation candidates before CIO approval.</p><div className="compactActions"><button type="button" disabled aria-disabled="true" title="Preview only — Opriva AI runs on the backend, not in the sandbox.">Review approvals</button><button type="button" disabled aria-disabled="true" title="Preview only — Opriva AI runs on the backend, not in the sandbox.">View consolidation</button><button type="button" disabled aria-disabled="true" title="Preview only — Opriva AI runs on the backend, not in the sandbox.">Prepare CIO report</button></div></section> : <section className="panel aiInsightBar assetsInsightBar"><p><strong>AI Insight (sample)</strong> Opriva found 12 records entering a critical renewal window and 18 records without an assigned owner. Start by assigning owners to high-value records expiring in the next 30 days.</p><div className="compactActions"><button type="button" disabled aria-disabled="true" title="Preview only — Opriva AI runs on the backend, not in the sandbox.">Assign owners</button><button type="button" disabled aria-disabled="true" title="Preview only — Opriva AI runs on the backend, not in the sandbox.">Review critical</button><button type="button" disabled aria-disabled="true" title="Preview only — Opriva AI runs on the backend, not in the sandbox.">Prepare emails</button></div></section>}
...
const ALERT_MODULE_NAV = { licenses: 'Licenses', hardware: 'Hardware', contracts: 'Contracts', documents: 'Documents', tasks: 'Tasks' };
              ...
              return <article key={a.id} style={{border:'1px solid #EEF2F7',borderLeft:'3px solid '+(ALERT_SEV_COLOR[a.severity]||'#64748B'),borderRadius:12,padding:'12px 14px',display:'grid',gap:5,background:a.seen?'#FBFCFE':'#fff'}}>
                ...
                <div style={{display:'flex',gap:6,flexWrap:'wrap',marginTop:4}}>
                  {filter === 'Resolved'
                    ...
                    : <>
                        <button type="button" style={actBtn} onClick={function(){ if (onNavigate) onNavigate(ALERT_MODULE_NAV[a.module] || 'Dashboard'); }}>Open record</button>
                        {createdFor[a.id]
                          ? <span style={{fontSize:12,color:'#0F766E',alignSelf:'center',fontWeight:700}}>Task created ✓</span>
                          : <button type="button" style={actBtn} onClick={function(){ if (onCreateTask) onCreateTask(a); setCreatedFor(function(p){ var n=Object.assign({},p); n[a.id]=true; return n; }); }}>Create task</button>}
...
function App(){
  ...
  const createTaskFromAlert = (alert) => {
    ...
    const title = 'Resolve alert: ' + alert.title;
    const task = {
      id: 'task-' + Date.now() + '-' + Math.random().toString(36).slice(2, 7),
      moduleKey: 'tasks', source: 'localAlert', title: title, taskType: 'Alert follow-up',
      owner: (alert.owner && alert.owner !== 'Unassigned') ? alert.owner : 'Unassigned',
      dueDate: alert.dueDate || today,
      priority: alert.severity === 'critical' ? 'Critical' : (alert.severity === 'high' ? 'High' : 'Medium'),
      status: 'Open', notes: alert.detail || '',
      sourceModule: alert.module || '', sourceRecordId: alert.recordId || '', sourceRecordName: alert.recordName || '',
      sourceClientOrDepartment: alert.clientOrDept || '', sourceAlertId: alert.id,
      workspaceMode: workspaceMode, createdAt: today,
    };
    const taskRow = [task.title, alert.clientOrDept || '-', alert.recordName || '-', alert.module || 'alert', task.taskType, task.owner, task.priority, task.dueDate, task.status, 'Open task'];
    if (!Array.isArray(RECORD_STORE.tasks)) RECORD_STORE.tasks = [];
    RECORD_STORE.tasks.push({ id: task.id, row: taskRow, meta: task });
    addActivityEvent({ eventType: 'task_created', title: 'Task created from alert', description: title, sourceModule: task.sourceModule, sourceRecordId: task.sourceRecordId, sourceRecordName: task.sourceRecordName, workspaceMode: workspaceMode });
    bumpAlerts();
  };
  ...
  return <div className={cx('app', sidebarCollapsed && 'appSidebarCollapsed', active === 'Expirations' && 'assetsRouteActive', active === 'Search' && 'searchRouteActive')}>
    ...
    <AlertsDrawer open={alertsOpen} workspaceMode={workspaceMode} onClose={() => setAlertsOpen(false)} onChange={bumpAlerts} onNavigate={(id) => { setActive(id); setAlertsOpen(false); }} onCreateTask={createTaskFromAlert} />
